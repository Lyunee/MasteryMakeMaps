...
def allDraw(H,graphname,getOldPos,dotORimg,img_size) :
# get positions
	...
	if dotORimg == "dot":
		...
		drawImgs(H,pos,values,graphname,img_size)
	else :
...
## the whole graph
	if sys.argv[2] == "all":
		...
		allDraw(H,graphname,sys.argv[4],sys.argv[3],0.02)
## groups
	...
	# for each group
		for gr in groups.keys():
			...
			allDraw(G,graphname+"_gr"+str(gr),sys.argv[4],sys.argv[3],0.03)
			print"save the graph for group " + str(gr)
			nx.write_gpickle(G, graphname+'_gr'+str(gr)+'.txt')
	else :
		print "\nThe second argument must be 'all' or 'groups'"
